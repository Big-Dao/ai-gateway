...
async fn tenant_admin_can_create_provider() {
    let server = TestServer::spawn_with_keys(&[
        ("admin-key", "default", "tenant_admin"),
    ]).await;
...
async fn admin_can_create_and_list_tenants() {
    ...
    assert_eq!(resp.status(), reqwest::StatusCode::CREATED, "admin creates tenant, got {}", resp.status());
    ...
    assert!(tenants.iter().any(|t| t["id"] == "acme-corp"), "new tenant should appear");
}

#[tokio::test]
async fn tenant_admin_cannot_create_tenant() {
    let server = TestServer::spawn_with_keys(&[
        ("ta-key", "default", "tenant_admin"),
    ]).await;
